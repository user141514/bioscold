...
Matched molecular pair (MMP)-derived fragment replacement ranking provides a scalable structure-derived benchmark for observed substitution patterns, but permissive splits can allow repeated fragment-attachment contexts across training and evaluation. We therefore study closed-vocabulary scaffold-conditioned replacement ranking under query-side transform-heldout evaluation, where the same old-fragment/attachment-context key is not shared across train and blind partitions. This is a small closed-vocabulary recovery benchmark: candidates are drawn from the training-derived replacement vocabulary rather than from an open or chemically enumerated replacement universe.
...
On Fresh Blind2, the 82-feature HistGB scorer achieves Top-10 = 0.8480, exceeding Score Blend (0.8077) and Borda(DE,HGB) (0.8176). The no-prior-rank 77-feature configuration remains baseline-beating (Top-10 = 0.8411) and slightly improves MRR, but its Top-10 is lower than the 82-feature scorer (Delta = -0.0070, 95% CI [-0.0094, -0.0044]). We therefore treat prior_ranks as an unstable query-relative feature family rather than as a cleanly transferable deletion target.

The resulting contribution hierarchy is benchmark first, prospective candidate-level scoring second, and diagnostic signal-transfer analysis third. Borda fusion explains why DE and HGB signals are complementary without becoming the manuscript endpoint. A4C and activity-comparable analyses remain workflow or boundary diagnostics, not validation endpoints.
...
**Leakage verification.** Table 3 reports overlap counts across split pairs for four relatedness dimensions: query-side transform-key overlap, old-fragment overlap, old-to-replacement pair overlap $(f^{\mathrm{old}}, c)$ excluding attachment signature, and attachment-signature overlap. Query-side transform-key overlap is zero for all evaluation pairs. The split removes exact query-side old-fragment/attachment-context overlap. It does not remove all old-fragment, replacement-pair, or attachment-signature relatedness.
...
|---|---:|---:|---:|---:|
| Train / Development set | 0 | -- | -- | -- |
| Train / Secondary blind | 0 | -- | -- | -- |
| Train / Canonical test | 0 | -- | -- | -- |
| Canonical / Secondary blind | 0 | -- | -- | -- |
| Train2 / Dev2 | 0 | 53 | 393 | 5 |
| Train2 / Blind2 | 0 | 46 | 430 | 5 |
| Dev2 / Blind2 | 0 | 17 | 188 | 5 |

"--" indicates quantities not computed for the original split pairs using the current overlap-definition conventions. All Fresh Blind2 split pairs have zero query-side transform-key overlap, confirming the transform-heldout property.
...
The following provenance table records where base-ranker scores and candidate-level score columns are fit, selected, generated, and applied. Dev2 base-ranker score columns are fitted-model scores rather than out-of-fold predictions unless explicitly stated; they are used for candidate-level fitting and diagnostics, not as independent Dev2 performance estimates.

---

**Provenance table.** Base-ranker and feature-generation provenance for Fresh Blind2.
...
A4C annotations are reported only in Supplementary Methods SM2 and Supplementary Table S9 as workflow-level diagnostics. They are not used for fitting, feature selection, ranking claims, activity validation, safety scoring, or medicinal-chemistry validation. They remain useful for documenting coverage-limited downstream review categories, but they do not affect the Fresh Blind2 performance hierarchy.
...
Query-level paired bootstrap intervals support the Fresh Blind2 candidate-level scoring result, but group-level analyses give wider Top-10 intervals because the blind queries are clustered by old fragments, transform keys, and attachment signatures. HistGB82 improved over Score Blend by +0.0403 Top-10 with a query-level 95% CI of [0.0355, 0.0446]. HistGB77 also improved over Score Blend, but the 77-minus-82 Top-10 comparison was negative at query level. Group-level Top-10 intervals can cross zero for candidate-level comparisons, whereas MRR sensitivity is more stable (Table 7); this prevents uniform fragment-level generalization claims.
...
|---|---|---:|---:|---:|---:|---|
| HistGB82 - Score Blend | Top-10 | +0.0403 | [+0.0355, +0.0446] | [-0.0068, +0.0734] | [-0.0101, +0.0717] | query-level gain; group Top-10 uncertainty wider |
| HistGB82 - Score Blend | MRR | +0.1141 | [+0.1099, +0.1184] | [+0.0852, +0.1505] | [+0.0841, +0.1491] | MRR sensitivity remains positive |
...
| HistGB77 - Score Blend | Top-10 | +0.0334 | [+0.0284, +0.0381] | [-0.0119, +0.0649] | [-0.0131, +0.0633] | baseline-beating query-level diagnostic; group Top-10 uncertainty wider |
...
### 4.5 DE/HGB Complementarity as a Secondary-Blind Diagnostic

The original secondary-blind diagnostic shows that DE and HGB occupy complementary hit regions, consistent with why rank fusion and candidate-level integration may remain useful. The two base rankers agree on many easy queries, but each also rescues queries missed by the other (Table 8). This table is diagnostic support for complementarity rather than primary Fresh Blind2 performance evidence.

**Table 8. Secondary-blind DE/HGB Top-10 hit overlap diagnostic.** Counts are query-level Top-10 hit indicators from the original 13,347-query secondary blind set.
...
Candidate-matrix audits support the traceability of the benchmark but do not justify over-claiming beyond the evaluated candidate universes. The Fresh Blind2 full-161 matrix is the primary policy because it preserves Blind2 positives without creating zero-positive queries. The original fixed-150 construction remains useful for comparability with the earlier secondary-blind benchmark, and within-vocabulary sensitivity analyses are reported in Supplementary Methods SM5 and Supplementary Table S8. We do not claim robustness to all possible full-compatible, fixed-300, external, or hard-negative candidate universes unless explicitly reported there.

### 4.7 Boundary Diagnostics: Activity-Comparable Reranking and Calibration Sanity

A retrospective activity-comparable diagnostic was used only to test the boundary between structure-derived recovery and biological relevance. Structure-derived positives can be linked to same-assay activity-comparable pairs and reranked by the candidate scorers, but degraded pairs can also receive high ranks. This diagnostic therefore does not support activity-outcome, biological-validation, or medicinal-chemistry prioritization claims. The benchmark remains a structure-derived replacement recovery task. Details are reported in Supplementary Methods SM3 and Supplementary Table S10.

Calibration diagnostics show that raw HistGB outputs should be interpreted as ranking scores rather than absolute probabilities. The models are therefore evaluated by ranking metrics and paired ranking deltas, not by absolute-score decision thresholds. Details are reported in Supplementary Methods SM4 and Supplementary Table S11.
...
The analysis uses ChEMBL37K, a curated subset of ChEMBL33. The V5/Fresh Blind2 evidence archive is frozen under repository tag `v5-fresh-blind2-evidence-lock-20260603` at https://github.com/user141514/paper1/tree/v5-fresh-blind2-evidence-lock-20260603, with the curated experiment-code folder at `bioisosteric_diffusion/paper1_experiment_code_archive`.
...
Large processed candidate matrices and any redistribution-restricted ChEMBL-derived artifacts will be deposited or documented separately before publication, subject to the redistribution terms of the underlying ChEMBL-derived data. An archival DOI will be provided upon acceptance.
