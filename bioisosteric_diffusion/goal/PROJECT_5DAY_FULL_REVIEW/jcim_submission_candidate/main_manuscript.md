...
We address these issues with two main contributions and two supporting safeguards. First, we define a transform-heldout benchmark with a secondary blind evaluation protocol (13,347 queries) that isolates unseen fragment-attachment combinations and prevents memorization of transform identities. Second, we introduce a candidate-level scorer, a HistGB model operating on chemically motivated feature families, that learns per-candidate trustworthiness from feature interactions at finer granularity than query-level gating. Supporting analyses show that parameter-free Borda fusion exposes complementarity between a Dual Encoder and a feature-based histogram gradient-boosted ranker (HGB), and an A4C computational review proxy annotates provenance and alert strata so that exploratory outputs are reported with explicit caveats rather than as unconditional medicinal-chemistry claims.

On the secondary blind evaluation protocol, the initial 82-feature scorer achieves Top-10 = 0.8851, whereas the post-audit 77-feature scorer reaches Top-10 = 0.9243. Removing the five prior_ranks features improves Top-10 by 3.93 pp over the 82-feature scorer (95% CI [3.54, 4.32] pp) and reduces Score Blend hit loss from 596 to 101 queries. Because this deletion was identified after blind diagnostics, we treat the 77-feature model as a post-audit locked result and the prior-rank interpretation as an analytical finding. Under the A4C proxy, G2 and G3 were fully covered and showed alert rates of 46.85% and 9.67%, respectively, whereas G4 had sparse A4C coverage (5.63%); no reliable G4-wide alert estimate is possible because 94.37% of G4 candidates have unknown A4C status.

Our contributions are as follows. First, we provide a leakage-controlled benchmark for closed-vocabulary, MMP-derived scaffold-conditioned fragment replacement with a transform-heldout split and secondary blind evaluation protocol. Second, we present a candidate-level scorer that improves over base-ranker fusion and show, through a post-audit ablation with paired bootstrap uncertainty, that sparse prior-rank features can become non-transferable shortcuts. Third, we enforce strict protocol separation: prospective performance evidence, post-audit mechanism analysis, and workflow interpretation each occupy distinct evidentiary roles. Fourth, we frame exploratory outputs through a dual-mode workflow that keeps provenance and alert stratification explicit while acknowledging the computational proxy's limitations.
