...
**Leakage verification.** Table 3 reports overlap counts across split pairs for four relatedness dimensions: query-side transform-key overlap, old-fragment overlap, full old-to-replacement triples $(f^{\mathrm{old}}, \sigma, c)$, and attachment-signature overlap. Query-side transform-key overlap is zero for all evaluation pairs. The split removes exact query-side old-fragment/attachment-context overlap. It does not remove all old-fragment, replacement-triple, or attachment-signature relatedness.

---

**Table 3.** Leakage and overlap verification across split pairs.

| Split Pair | Query-side $(f^{\mathrm{old}}, \sigma)$ overlap | Old-fragment overlap | Full old-to-replacement overlap | Attachment-signature overlap |
|---|---:|---:|---:|---:|
...
**Overlap verification.** Query-side $(f^{\mathrm{old}}, \sigma)$ overlap is zero for the transform-heldout evaluation pairs used for claims (Table 3). Residual old-fragment and replacement-triple relatedness are tracked as diagnostic leakage-scope quantities rather than treated as eliminated.
...
### 4.7 Learning-to-Rank Diagnostics Suggest a Future Architecture Path
