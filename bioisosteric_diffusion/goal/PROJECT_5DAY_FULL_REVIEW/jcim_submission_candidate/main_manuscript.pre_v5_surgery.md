...
We organize the study around three evidentiary layers. First, we define a transform-heldout benchmark with a secondary blind evaluation protocol (13,347 queries) that isolates unseen fragment-attachment combinations and prevents memorization of transform identities. Second, we introduce a candidate-level scorer, a HistGB model operating on chemically motivated feature families, that learns per-candidate trustworthiness from feature interactions at finer granularity than query-level gating. Third, we diagnose prior_ranks as query-relative shortcut features whose apparent usefulness does not transfer cleanly under secondary blind fragment shift. Borda(DE,HGB) is retained as a complementarity and mechanism anchor, A4C is reported only as a supporting computational triage workflow, and query-level routing is treated as a negative diagnostic rather than as a contribution.

On the secondary blind evaluation protocol, the initial 82-feature scorer achieves Top-10 = 0.8851, whereas the post-audit 77-feature scorer reaches Top-10 = 0.9243. Removing the five prior_ranks features improves Top-10 by 3.93 pp over the 82-feature scorer (95% CI [3.54, 4.32] pp) and reduces Score Blend hit loss from 596 to 101 queries. Because this deletion was identified after blind diagnostics, we treat the 77-feature model as a post-audit locked result and the prior-rank interpretation as an analytical finding. As a workflow-only supplement, the A4C proxy reports provenance-group alert strata with explicit coverage caveats; these signals do not constitute medicinal-chemistry validation or safety prediction.

The resulting contribution hierarchy is therefore benchmark first, candidate-level scoring second, and prior_ranks shortcut diagnosis third. This ordering is important: Borda fusion explains why DE and HGB signals are complementary but is not the final method; A4C organizes exploratory proposals but is not a validation endpoint; and query-level routing is included only because its failure clarifies why the final scorer evaluates query--candidate pairs rather than choosing one ranker per query.
...
The base ranker architecture, scorer feature families F1--F9, and training hyperparameters were configured on a held-out development/calibration set drawn from the training partition (Section 3.2). The prior_ranks feature family was removed after post-audit analysis of the initial secondary blind evaluation revealed unexpected per-fragment degradation; the mechanistic basis for removal is documented in Section 3.5.2. The 77-feature configuration described in Section 3.4.2 is the **post-audit locked model** in this study, with the caveat that the prior_ranks removal is a post-selection feature-pruning step rather than a fully prospective pre-registered feature-selection result. All performance results appear in Section 4.
...
The five prior_ranks features are excluded from the final model. The locked configuration uses the 77-feature set described in Section 3.4.2. Ablation results are reported in Section 4.3. Final model performance is reported in Section 4.2. Rescue and lost analysis methodology is defined in Section 3.6.3; results appear in Section 4.4.
...
Aggregated counts $R(m,b)=\sum_i\operatorname{Rescue}_i$, $L(m,b)=\sum_i\operatorname{Lost}_i$, and net gain $G=R-L$ decompose accuracy changes into recoveries and regressions. Results are reported in Section 4.4.
...
**Table M5.** Provenance groups and definitions. Alert rates are reported in Section 4.6.
...
We evaluate all methods on the secondary blind protocol (13,347 queries; Section 3.2) using query-level Top-10 accuracy as the primary metric, with 95% bootstrap confidence intervals (5,000 query-level resamples; Section 3.6.2). Section 4.1 first tests why transform-heldout evaluation is necessary by comparing locked base rankers under a permissive random split. Section 4.2 then reports candidate-level scoring under the transform-heldout secondary blind protocol. Sections 4.3 and 4.4 analyze the post-audit prior_ranks deletion and its rescue/lost profile. Section 4.5 documents a categorical schema alignment audit. Section 4.6 reports A4C provenance-group alert stratification.

---

### 4.1 Permissive Splits Inflate Learned Ranking Signals

A split stress test shows why the transform-heldout benchmark is necessary. A random/permissive query split preserved the old-test quarantine and train-blind query non-overlap, but allowed train-blind transform-key overlap for 840/840 blind transform identities. Under this permissive overlap, DE increased from 0.8055 to 0.8784 (Delta = +0.0729, 95% CI [0.0643, 0.0814]), HGB increased from 0.7437 to 0.8191 (Delta = +0.0755, 95% CI [0.0655, 0.0853]), and Borda(DE,HGB) increased from 0.8384 to 0.8699 (Delta = +0.0316, 95% CI [0.0230, 0.0398]); Attachment-Frequency decreased from 0.6019 to 0.5727. Because Score Blend and candidate-level scorers were not rebuilt under the random split, this diagnostic is not a competing benchmark. It shows that permissive transform overlap changes which learned base-ranker signals appear transferable (Supplementary Table S6).

The random/permissive stress test does not show that every method is inflated by random splitting, because Attachment-Frequency decreases under this split. Instead, it isolates the main benchmark risk for learned rankers: when transform identities cross train and evaluation partitions, structural and empirical learned signals can appear substantially stronger than they do under transform-heldout evaluation. We therefore treat the transform-heldout secondary blind split as the only source of primary performance claims.

### 4.2 Candidate-Level Scoring Improves Top-10 Recovery under Transform-Heldout Evaluation

Candidate-level scoring improves blind ranking over the base-ranker fusion baselines, but the evidence hierarchy differs for the 82-feature and 77-feature versions. The initial 82-feature scorer is the prospective candidate-level blind result and achieves Top-10 = 0.8851 on 13,347 secondary blind queries. The post-audit 77-feature scorer, obtained after removing prior_ranks features based on blind diagnostics, achieves Top-10 = 0.9243. It improves over the strongest pre-D4S baseline, the MLP+HGB Score Blend, by Delta Top-10 = +0.0686 using unrounded query-level hit indicators, with a 95% paired bootstrap CI of [+0.0638, +0.0733]. Because the prior_ranks removal was prompted by blind-set diagnostics, the 77-feature result is reported as a locked post-selection result rather than a fully prospective feature-selection result.
...
This pattern identifies prior_ranks as a non-transferable shortcut. These features encode the within-query rank positions of sparse prior scores. Such ranks are predictive within the development distribution, but their meaning changes when the fragment composition shifts from the 8-fragment development setting to the 19-fragment secondary blind setting. Removing them forces the model to rely more heavily on candidate-level signals with more stable cross-split meaning relative to prior_ranks: raw model scores, train-derived support-aware statistics, molecular descriptors, fingerprint similarities, and categorical features under the frozen schema.

To test whether the prior-rank effect reflected a rank-specific shortcut rather than generic removal of prior information, we ran targeted prior-family ablations under the fixed HistGB protocol. Dropping prior_ranks produced the largest Top-10 increase relative to the 82-feature scorer (+0.0393, 95% CI [0.0354, 0.0432]). By comparison, dropping all non-rank prior families produced a smaller increase (+0.0174), dropping prior statistics gave +0.0151, dropping prior scores gave +0.0073, and dropping PMI/conditional-prior contrasts reduced Top-10 (-0.0037, 95% CI [-0.0064, -0.0010]). A support-bin audit showed that the 77-feature gain was largest in low-support queries (+0.0488), compared with medium- and high-support queries (+0.0344 and +0.0346). These diagnostics support a bounded interpretation: the dominant unstable prior-family component is the query-relative prior-rank transform, not prior information in general.
...
### 4.4 Feature Pruning Recovers Baseline Misses While Reducing Regression
...
### 4.6 Dual-Mode Provenance Strata Provide Coverage-Limited Triage Signals
...
This study makes three central empirical contributions. First, under a leakage-controlled transform-heldout benchmark with 13,347 secondary blind queries, the prospective 82-feature candidate-level scorer achieves Top-10 = 0.8851, and the post-audit feature-pruned 77-feature scorer achieves Top-10 = 0.9243, improving over the Score Blend baseline by Delta Top-10 = +0.0686 [95% CI: +0.0638, +0.0733] (Table 1). Second, the main audit finding is that query-relative prior-rank features behave as non-transferable shortcuts under secondary blind fragment shift: removing five per-query prior-rank features improves blind Top-10 by +0.0393 [95% CI: +0.0354, +0.0432] over the 82-feature scorer (Table 2). Because this deletion was identified after blind diagnostics, the ablation result is analytical rather than fully prospective. Third, beyond ranking performance, the dual-mode workflow provides a coverage-limited computational triage layer: provenance labels stratify Exploration Mode proposals by computational alert burden, with complete-coverage G2/G3 rates of 46.85% and 9.67%, while G4 remains too sparsely covered for a reliable group-wide estimate (Table 4).
...
### 5.2 Query-Level Routing as a Negative Diagnostic

Query-level routing was retained only as a negative diagnostic, not as a manuscript contribution. A runtime-safe Borda/HGB router selected on development rules routed 884 of 13,347 blind queries but did not improve Top-10 over Borda(DE,HGB) (Delta = 0.0000, 95% CI [-0.0008, 0.0008]) and reduced MRR by 0.0041. A later learned query router reached only about 0.61 validation AUC and did not yield a clean blind method. These failures support a narrow design lesson: in this benchmark, useful evidence is better exploited by scoring each query--candidate pair than by making a single query-level method-selection decision.

### 5.3 Prior_Ranks as Non-Transferable Shortcuts

Post-audit analysis indicates that query-relative prior-rank features behaved as non-transferable shortcuts under the secondary blind fragment shift. The largest targeted ablation gain came from removing prior ranks, the gain was strongest in low train-derived support regimes, and fragment-level regressions were reduced after pruning (Supplementary Tables S4 and S5). We interpret this as shortcut learning [16]: the model exploits a feature that appears informative during development but whose semantics are distribution-specific. Alternative explanations remain plausible, including multicollinearity among prior features, unstable rank transforms under sparse support, and threshold effects in the gradient-boosted trees under distribution shift.

This finding does not condemn rank features in general or prior information in general. The retained model-rank family (F3, encoding base-ranker rank positions) has different provenance from the removed prior_ranks: base-ranker ranks summarize trained model outputs, whereas prior_ranks derive from sparse empirical prior-score transforms whose rank distributions are tightly coupled to fragment composition and support. Relative to sparse query-relative prior-rank transforms, raw prior scores, support-aware statistics, PMI contrasts, molecular descriptors, and model outputs appeared more stable in this benchmark audit. This remains a bounded post-audit interpretation rather than a universal rule about rank features or a fully prospective feature-selection claim.
