...
The component ladder (Table 2) shows that a tuned frequency–content blend is already a strong baseline (0.797), substantially above content similarity alone (0.661). This confirms that combining track record with structural similarity is valuable. LBC-Ranker further improves over this blend by +0.055 (OF-macro), outperforming it in all 10 seeds. The mechanism is feature-resolved compatibility: CA compresses multiple structural dimensions (Morgan, five physicochemical deltas) into a single scalar, and a linear blend with frequency can only adjust the global weight of that scalar. LBC-Ranker retains each structural dimension individually and learns their joint calibration against candidate frequency, capturing information that coarse blending discards.
