...
To assess whether the findings generalize beyond the ChEMBL 36-derived training data, we evaluate LBC-Ranker and baselines on a hard-negative BindingDB-derived MMP benchmark (81,839 queries, 24,037 candidate fragments, 1,179 targets). Hard negatives were constructed by preferentially drawing candidate negatives from the same target, endpoint, and attachment signature as positives but with different core scaffolds, then selecting the most content-similar candidates — intentionally choosing look-alike non-replacements as stress-test negatives. Under a 3-seed repeated OF split, LBC-Ranker (OF-macro Hit@10 = 0.949) and FullBlend (0.952) both substantially outperform Frequency (0.783), CA (0.819), and Blend (0.861). The comparable LBC/FullBlend performance on this external benchmark is consistent with our ChEMBL finding that learned content support dominates the hard-negative ranking, with the feature-resolution advantage providing a smaller but reliable gain. These results confirm that the LBC-Ranker framework transfers across data sources and that easy-negative benchmarks (where CA alone reaches 0.995) should be avoided as uninformative. The pre-specified protocol and full 10-seed result lock for BindingDB are provided in the Supporting Information.
