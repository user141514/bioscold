...
### 3.1 Component Ladder: Simple Blending Is Insufficient

To test whether a simple linear blend of candidate frequency and content similarity suffices, we construct a component ladder under the same 10-seed protocol (Table 2). Under OF-macro Hit@10 (3-seed mean, with the full 10-seed confirmed in the Supporting Information), frequency alone achieves 0.448. Tuned content similarity (CA, λ = 0.75) reaches 0.621. A tuned linear blend of frequency and CA (α = 0.25 across all seeds) improves Hit@10 to only 0.644, a marginal gain of +0.023 over CA alone. In one seed the blend degrades performance relative to CA (−0.088). LBC-Ranker achieves 0.784, gaining +0.140 over the blend. The blend improves primarily under query-weighted evaluation (+0.118, where high-volume OFs dominate), while its OF-macro gain is negligible (+0.023, where each OF contributes equally). LBC-Ranker improves under both metrics (+0.140 macro, +0.050 query-weighted), demonstrating that learned feature-resolved compatibility benefits low-volume and high-volume OFs alike.

**Table 2: Component ladder (OF-macro Hit@10, 3-seed mean; query-weighted, 10-seed mean).**

| Method | OF-macro (3-seed) | Query-weighted (10-seed) |
|--------|-------------------|-------------------------|
| Frequency | 0.448 | 0.477 |
| CA (tuned) | 0.621 | 0.608 |
| Freq+CA blend (tuned) | 0.644 | 0.727 |
| **LBC-Ranker** | **0.784** | **0.777** |
