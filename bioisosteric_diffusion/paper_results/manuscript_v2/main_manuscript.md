...
The component ladder (Table 3) and primary results (Table 2) show that a tuned frequency–content blend is the strongest non-LBC baseline (0.797), substantially above both CA (0.661) and C3F-style retrieval (0.716). LBC-Ranker improves over this blend by +0.055 (OF-macro), outperforming it in all 10 seeds. The mechanism is feature-resolved compatibility: CA compresses multiple structural dimensions into a single scalar, and a linear blend with frequency can only adjust the global weight of that scalar. LBC-Ranker retains each structural dimension individually and learns their joint calibration against candidate frequency, capturing information that coarse blending discards. This improvement is modest in magnitude (+0.055) but consistent across all seeds, indicating that the feature-resolved representation provides a reliable, if incremental, gain over the strongest simple baseline.
